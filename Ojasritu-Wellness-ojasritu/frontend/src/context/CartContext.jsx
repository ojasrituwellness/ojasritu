...
export function CartProvider({ children }) {
  ...
  const items = useMemo(() => {
    ...
    return cartItems.map((ci) => {
      ...
      // Guard against malformed product payloads
      const safe = {
        name: typeof prod.name === 'string' ? prod.name : 'Product',
        price: Number(prod.price ?? 0) || 0,
        discount_price: Number(prod.discount_price ?? 0) || 0,
        image: typeof prod.image === 'string' ? prod.image : null,
        category_name: typeof prod.category_name === 'string' ? prod.category_name : undefined,
        sku: typeof prod.sku === 'string' ? prod.sku : undefined,
        slug: typeof prod.slug === 'string' ? prod.slug : undefined,
      };
      return { ...base, ...safe };
    });
  }, [rawCart?.items]);
